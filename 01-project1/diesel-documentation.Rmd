...
summary(mod_met_filtered %>% filter(mod_met_filtered$sn == 'MOD-PM-00222'))
```
...
mod_met_filtered$nox <- (mod_met_filtered$no + mod_met_filtered$no2)
mod_met_filtered$ratio <- mod_met_filtered$nox / mod_met_filtered$co
...
## Calendar plot
We saw a big dip in MOD-00028's NO2 level around November 14th, 2022. We think this may have been because the sensor was moved, but we didn't see any changes in the `lat` or `lon` variables.
```{r}
my_df <- mod_met_filtered %>%
      filter(mod_met_filtered$sn == 'MOD-00028')
calendarPlot(my_df, pollutant = 'no2', main='no2')
```
...
```{r}
summary(nubian_df)
sd(nubian_df$`no2_MOD-00024`, na.rm=TRUE)
sd(nubian_df$`no2_MOD-00025`, na.rm=TRUE)
sd(nubian_df$`no2_MOD-00028`, na.rm=TRUE)
```
