...
for (pollutant in c('ratio', 'pm1', 'co', 'nox')) {
  polarMap(nubian_df,
     pollutant = 'ratio',
     key.position = "bottom",
     key.header = 'ratio',
...
nubian_df$date <- round_date(nubian_df$date, unit = "minute")
nubian_df <- pivot_wider(nubian_df, id_cols = date, names_from = sn, values_from = c(no2, nox), values_fn = {mean})
names(nubian_df)
summary(nubian_df)
sd(nubian_df)
# timePlot(nubian_df, pollutant=c("MOD-00024", "MOD-00025", "MOD-00028"), ylim=c(0, 80), avg.time = "week", group=TRUE, lty=1)
```
